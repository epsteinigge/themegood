...
const slideVideoFileInput = document.getElementById("slideVideoFile");
const slideVideoUrlInput = document.getElementById("slideVideoUrl");
...
const uploadSlideVideoBtn = document.getElementById("uploadSlideVideoBtn");
...
function renderSlides(slides) {
  ...
  slides.forEach((slide) => {
    ...
    card.innerHTML = `
      <div class="admin-list-card-image">
        ${
          slide.video_url
            ? `<video src="${escapeHtml(slide.video_url)}" muted playsinline preload="metadata" controls></video>`
            : slide.image_url
      ...
      <div class="admin-list-card-body">
        ...
        <p><strong>Media:</strong> ${slide.video_url ? "Video" : "Image"}</p>
  ...
  homepageSlidesList.querySelectorAll("[data-action='edit-slide']").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      slideVideoUrlInput.value = slide.video_url || "";
      ...
      slideSecondaryLinkInput.value = slide.button_secondary_link || "#products";
...
function renderSections(sections) {
  ...
  const sectionGuidance = {
    about: "Controls the About ThemeGood title and lead paragraph.",
    signature_experience: "Controls the Signature Experience panel title and body.",
    flavour_collection: "Controls the Flavour Collection heading and intro.",
    featured_products: "Controls the Featured Products heading and intro.",
    bundles: "Controls the Bundles heading and paragraph.",
    testimonials: "Controls the Testimonials heading and intro.",
    faq: "Controls the FAQ heading and intro.",
    newsletter: "Legacy homepage newsletter copy. This may not appear in the current frontend."
  };

  visibleSections.forEach((section) => {
    const normalizedKey = String(section.section_key || "").trim().toLowerCase();
    const helperText = sectionGuidance[normalizedKey] || "Controls public homepage text for this section key.";
    ...
    wrapper.innerHTML = `
      <div class="section-card-head">
        <div>
          ...
          <p class="section-card-copy">${escapeHtml(helperText)}</p>
        </div>
      </div>
...
uploadSlideVideoBtn.addEventListener("click", async () => {
  const file = slideVideoFileInput.files?.[0];
  if (!file) {
  ...
  const response = await fetch("/api/upload-homepage-slide-video", {
    method: "POST",
    headers: {
      "x-admin-token": adminToken
    },
  ...
  slideVideoUrlInput.value = result.video_url || "";
  slideImageUrlInput.value = "";
...
slideForm.addEventListener("submit", async (event) => {
  ...
  const payload = {
    ...
    video_url: slideVideoUrlInput.value.trim(),
